...
class Ujian extends CI_Controller {
    ...
    {
        if ( !$this->ion_auth->in_group('Penilai') ){
	...
	public function save()
	{
		...
		$bil_prima 		= $this->input->post('bil_prima', true);
		...
		if( $this->form_validation->run() === FALSE ){
			...
			$data['errors'] = [
				...
				'total_soal' 	=> form_error('total_soal'),
				'jumlah_peserta'=> form_error('jumlah_peserta'),
				'bil_prima'		=> form_error('bil_prima'),
		...
		if( $this->input->post('jenis') =='acak' ){//ini error -> null , acak A nya kecil wkwk
		// echo $this->input->post('jenis');
			...
			/* generate th */
			$th= '';
			for ($s=1; $s < ($this->input->post('jumlah_peserta')+1) ; $s++) {//post bukan gett wkwk
			}

			$_lcg= [
			    'b'=> $this->input->post('bil_prima'),
			    'm'=> $this->input->post('total_soal')//awalnya total_soal
			];

			$this->benchmark->mark('code_start'); # mulai waktu pengacakan metode LCG
			for ($i=1; $i < ($this->input->post('jumlah_soal')+1) ; $i++) {
			    $tr.= '<tr>';
			    $tr.= "<td>$i</td>"; 
			    for ($j=1; $j < ($this->input->post('jumlah_peserta')+1) ; $j++) {
			        if ( $i==1 ) {
			...
			echo 'Hasil LCG = ' . json_encode($lcg);
			//nde sini ntar return view with data e itu $lcg
			// $this->load->view('auth/login', $this->lcg); // like this as an example
			// $this->output_json(['status'=>json_encode($lcg)]);
		}
